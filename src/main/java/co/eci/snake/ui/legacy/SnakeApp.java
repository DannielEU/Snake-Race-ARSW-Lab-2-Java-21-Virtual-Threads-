...
public final class SnakeApp extends JFrame {
  ...
  private void togglePause() {
    if ("Action".equals(actionButton.getText())) {
      ...
      clock.pause();
      // Al pausar, calculamos estadísticas consistentes usando snapshots sincronizados
      int idx = 0;
      int bestIdx = -1, bestLen = -1;
      int worstIdx = -1, worstLen = Integer.MAX_VALUE;
      for (Snake s : snakes) {
        int len = s.snapshot().size();
        if (len > bestLen) { bestLen = len; bestIdx = idx; }
        if (len < worstLen) { worstLen = len; worstIdx = idx; }
        idx++;
      }
      String msg = String.format(
          "Serpiente viva más larga: #%d (longitud=%d)\nPeor serpiente (más corta): #%d (longitud=%d)\n\nNota: no existe un mecanismo de 'muerte' en esta versión; 'peor' se interpreta como la más corta.",
          bestIdx, bestLen, worstIdx, worstLen);
      JOptionPane.showMessageDialog(this, msg, "Estadísticas (pausa)", JOptionPane.INFORMATION_MESSAGE);
    } else {
